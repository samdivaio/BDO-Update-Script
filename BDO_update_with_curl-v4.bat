...
for /F "tokens=7" %%a in ('findstr /rc:"client version: [0-9]*" "update.log"') do set "clientver=%%a"
ECHO Client version is: %clientver%
ECHO.
for /F "tokens=7" %%b in ('findstr /rc:"latest version: [0-9]*" "update.log"') do set "latestver=%%b"
